...
def create_database():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS destinations
                     (id INTEGER PRIMARY KEY AUTOINCREMENT, source TEXT, destination TEXT, date DATE, time_taken TEXT, expenses FLOAT, days INTEGER, status TEXT)''')
    conn.commit()
    conn.close()
...
class TravelPlannerApp(tk.Tk):
    def __init__(self):
        ...
        self.destination_table = ttk.Treeview(self, columns=("ID", "Source", "Destination", "Date", "Time Taken", "Expenses", "Days", "Status"), show="headings")
    ...
    def add_destination(self):
        ...
        if not all([source, destination, date, time_taken, expenses, days]):
        ...
        conn = sqlite3.connect('travel_planner.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO destinations (source, destination, date, time_taken, expenses, days, status) VALUES (?, ?, ?, ?, ?, ?, ?)", (source, destination, date, time_taken, expenses, days, status))
        conn.commit()
        conn.close()
    ...
    def view_destinations(self):
        ...
        columns = ("ID", "Source", "Destination", "Date", "Time Taken", "Expenses", "Days", "Status")
        destination_table = ttk.Treeview(view_window, columns=columns, show="headings")

        # Set column headings
        for col in columns:
            destination_table.heading(col, text=col)
            destination_table.column(col, width=100)  # Adjust column width as needed
